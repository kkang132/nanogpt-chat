...
GPT-2 (model.py) ─── 124M params, see docs/model.md
  ...
  └──→ eval.py ──→ eval/eval_results.jsonl (perplexity, generation, GSM8K)
...
**Evaluation**: discover checkpoints → run perplexity, generation quality, and GSM8K suites → format table → append to `eval/eval_results.jsonl`.
...
| `eval/eval_results.jsonl` | one JSON object per checkpoint evaluation | generated by eval.py |
